...
# Initialisation of the openBL URL and directory where data will be downloaded (make sure this directory exist)

allTrackerName = ["base_1day"]
allIpList = ["http://www.openbl.org/lists/base_1days.txt"]
allDownloadDirectory = ["../data/"]
...
if __name__ == "__main__":
    ...
    for tracker, ipList, dlDirectory in zip(allTrackerName,allIpList,allDownloadDirectory):
        ...
        try:
            ...
            client.execute("create table if not exists openbl_%s (ip string, hostname string) partitioned by(dt string) row format delimited fields terminated by '\t'" % tracker);
            client.execute("load data local inpath '{csvFile}' overwrite into table openbl_{tracker} partition (dt='{date}')".format(csvFile=csvFilename,date=dt,tracker=tracker))
            transport.close()
